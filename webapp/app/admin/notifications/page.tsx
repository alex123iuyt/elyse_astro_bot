  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <div className="p-4 border-b border-zinc-800 flex items-center justify-between">
        <h1 className="text-xl font-semibold">Создать рассылку</h1>
      </div>

      <div className="p-4 max-w-4xl mx-auto">
        <div className="bg-zinc-900 rounded-xl border border-zinc-800 flex flex-col h-[80vh]">
            ...
            {progress && (
              <div className="mt-4">
                ...
                <div className="w-full h-2 bg-zinc-800 rounded">
                  <div className="h-2 bg-emerald-600 rounded" style={{ width: `${Math.min(100, Math.floor((progress.sent / Math.max(1, progress.total)) * 100))}%` }}></div>
                </div>
                {progress.failed > 0 && <div className="text-red-400 text-sm mt-2">Ошибок: {progress.failed}</div>}
              </div>
            )}
          </div>
        </div>
